...
impl HtmlTokenizer {
    ...
    // create_tagメソッドによって作られた最後のトークンに属性の文字を追加する
    ...
    // create_tagメソッドによって作られた最後のトークンが開始タグの場合、self_closingフラグを有効にする
        ...
        loop {
            ...
            match self.state {
                ...
                State::ScriptDataLessThanSign => {
                    ...
                    // <script>終了タグではなかったのでJavaScriptのコードとしての処理を続ける
                    self.reconsume = true;
                ...
                State::ScriptDataEndTagOpen => {
                    ...
                    self.reconsume = true;
                    self.state = State::ScriptData;
                    // 仕様だと２文字のトークンを返すようだが、今回の実装は1文字のトークンしか返せないため仕様と違う実装になっている
                    return Some(HtmlToken::Char('<'));
                }
                // JavaScriptの終了を表す</script>終了タグの名前部分(script)を解析している状態
                State::ScriptDataEndTagName => {
                    ...
                    self.buf = String::from("<") + &self.buf + &c.to_string();
                    self.buf.push(c);
                    continue;
                }
                // 一時的にデータを蓄えるための状態
...
mod tests {
    ...
    fn test_self_closing_tag() {
        let html = "<img alt='abc' />".to_string();
        let mut tokenizer = HtmlTokenizer::new(html);
        let mut attr1 = Attribute::new();
        attr1.add_char('a', true);
        attr1.add_char('l', true);
        attr1.add_char('t', true);
        attr1.add_char('a', false);
        attr1.add_char('b', false);
        attr1.add_char('c', false);

        let expected = [
            HtmlToken::StartTag {
                tag: "img".to_string(),
                self_closing: true,
                attributes: vec![attr1],
            }
        ];
